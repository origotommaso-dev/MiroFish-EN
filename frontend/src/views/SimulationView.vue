...
const handleNextStep = (params = {}) => {
  ...
  // Log simulation round count configuration
  if (params.maxRounds) {
  ...
  // If custom rounds specified, pass via query parameters
  if (params.maxRounds) {
...
const checkAndStopRunningSimulation = async () => {
  ...
  try {
    ...
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog('Running simulation environment detected, shutting down...')
      ...
      try {
        ...
        if (closeRes.success) {
          addLog('Simulation environment closed successfully')
        } else {
        ...
        // If graceful close threw exception, try force stop
        await forceStopSimulation()
      }
    } else {
    ...
    // Failure to check environment status doesn't affect subsequent flow
...
const forceStopSimulation = async () => {
  try {
    ...
    if (stopRes.success) {
      addLog('Simulation force stopped successfully')
    } else {
...
onMounted(async () => {
  ...
  // Check and close running simulations (when user returns from Step 3)
