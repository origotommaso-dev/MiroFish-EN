<template>
  <div class="process-page">
    ...
    <div class="main-content">
      <!-- Left Side: Real-time Graph Visualization -->
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (display as long as data exists) -->
...
// Initialize - handle new project creation or load existing project
...
const handleNewProject = async () => {
  ...
  if (!pending.isPending || pending.files.length === 0) {
    error.value = 'No pending files found. Please go back to the homepage and try again.'
    loading.value = false
    return
  }
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling tasks that are building
  ...
  try {
    ...
    if (response.success) {
      ...
      if (task.status === 'completed') {
        ...
        // Update progress display to completion state
        buildProgress.value = {
          progress: 100,
