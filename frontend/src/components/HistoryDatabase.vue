    ...
    <div v-if="projects.length > 0" class="cards-container" :class="{ expanded: isExpanded }" :style="containerStyle">
      ...
      >
        ...
        <div class="card-files-wrapper">
          ...
          <!-- Empty placeholder when no files -->
        ...
        <!-- Card title (first 20 characters of simulation requirement as title) -->
        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-playback-hint">
              <span class="hint-text">Step3 "Start Simulation" and Step5 "Deep Interaction" must be launched while running and do not support history replay</span>
            </div>
          </div>
        </div>
      </Transition>
...
let pendingState = null  // Record target state to be executed
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate card count per row, ensure each row is centered
    ...
    const x = offset * 35
    // Adjust start position, close to title but maintain proper spacing
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update target state (record latest target state regardless of animation)
        ...
        // If animating, just record state, process after animation ends
...
.grid-pattern {
  ...
  /* Position from top-left, height changes only expand at bottom, not affecting existing grid */
...
.cards-container {
  ...
  /* min-height dynamically calculated by JS, auto-adapts to card count */
}
