    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">Some content is still being processed. We recommend manually refreshing the graph later.</span>
...
const wasSimulating = ref(false) // Track whether it was previously simulating
...
const renderGraph = () => {
  ...
  // Process edge data, calculate edge count and index between each node pair
  const edgePairCount = {}
  ...
  tempEdges.forEach(e => {
    ...
    if (totalCount > 1) {
      ...
      // This ensures all edges distribute in the same reference frame, preventing overlap
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  const simulation = d3.forceSimulation(nodes)
    ...
    // Add gravitational pull toward center to cluster independent node groups
  ...
  const getLinkPath = (d) => {
    ...
    // Detect self-loop
    if (d.isSelfLoop) {
      // Self-loop: draw an arc from the node back to itself
      const loopRadius = 30
    ...
    // Offset perpendicular to link direction, proportional to distance for visible curves
    // More edges means larger proportion of offset relative to distance
