...
const toolConfig = {
  ...
  'interview_agents': {
    ...
    color: 'green',
    icon: 'users' // users icon - represents dialogue
  },
  'quick_search': {
    name: 'Quick Search',
    color: 'orange',
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    // Extract sub-questions - full extraction, unlimited
    ...
    // Extract key facts - full extraction, unlimited
    ...
    // Extract relation chains - full extraction, unlimited
...
const parsePanorama = (text) => {
  ...
  try {
    ...
    // Extract current active facts - full extraction, unlimited
    ...
    // Extract historical/expired facts - full extraction, unlimited
    ...
    // Extract involved entities - full extraction, unlimited
  ...
  try {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        // e.g.: 1. **alumni_345（index=1）**: As a university alumni...
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (qMatch) {
        ...
        // Split questions by number index
        const questions = qText.split(/\n\d+\.\s+/).filter(q => q.trim())
        if (questions.length > 0) {
          // If first question is preceded by "1.", needs special handling
      ...
      if (answerMatch) {
        ...
        // Platform fallback logic (compatible with old format: single platform tag case)
...
const parseQuickSearch = (text) => {
  ...
  try {
    ...
    // Extract related facts - full extraction, unlimited
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    // Check if it's a platform placeholder text
    const isPlaceholderText = (text) => {
      if (!text) return true
      const t = text.trim()
      return t === '(No response received on this platform)' || t === '(该平台未获得回复)' || t === '[无回复]'
    }
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // If no numbering found or only one, return as whole
      if (matches.length <= 1) {
              ...
              answerText && h('div', { class: ['qa-answer', { 'answer-placeholder': isPlaceholder }] }, [
                ...
                h('div', { class: 'qa-content' }, [
                  h('div', { class: 'qa-answer-header' }, [
                    ...
                    // Dual platform toggle buttons (shown only when both platforms have real answers)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading level-2 heading (## xxx), since section title is shown in the outer layer
  ...
  // Clean <p><br> following block elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> before paragraph start tags that follow block elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: maintain incrementing numbers when single-item <ol> are separated by paragraph content
...
const extractFinalContent = (response) => {
  ...
  // If content is long and contains markdown, try removing thought process and return
