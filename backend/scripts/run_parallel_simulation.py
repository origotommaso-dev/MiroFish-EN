...
- Does not immediately close environment after simulation, enters command waiting mode
...
if sys.platform == 'win32':
    ...
    # Reconfigure stdout to UTF-8 (fix console character encoding)
    if hasattr(sys.stdout, 'reconfigure'):
        sys.stdout.reconfigure(encoding='utf-8', errors='replace')
    if hasattr(sys.stderr, 'reconfigure'):
        sys.stderr.reconfigure(encoding='utf-8', errors='replace')
    
    # Force set default encoding (affects open() default encoding)
    # Note: this needs to be set at Python startup, may not take effect at runtime
    ...
    def _utf8_open(file, mode='r', buffering=-1, encoding=None, errors=None, 
        ...
        This fixes third-party libraries (e.g., OASIS) reading files without specifying encoding
        """
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logging every agent observation and action), we use our own action_logger
    """
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize simulation logging config
    
    Args:
        simulation_dir: Simulation directory path
    """
    # Disable verbose OASIS logs
...
# Twitter available actions (excludes INTERVIEW, which can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Reddit available actions (excludes INTERVIEW, which can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    def _get_env_and_graph(self, platform: str):
        """
        Get env and agent_graph for specified platform
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Specified platform (optional)
                ...
                - None/unspecified: Interview both platforms, return combined results
            
        Returns:
            True for success, False for failure
        """
        # If platform specified, only interview that platform
        ...
        if not self.twitter_env and not self.reddit_env:
            self.send_response(command_id, "failed", error="No available simulation environments")
            return False
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Default platform (can be overridden per interview item)
                - "twitter": Interview Twitter platform only
                - "reddit": Interview Reddit platform only
                - None/unspecified: Each Agent interviewed on both platforms
        """
        # Group by platform
        twitter_interviews = []
        reddit_interviews = []
        both_platforms_interviews = []  # Needs interview on both platforms
...
# Non-core action types to filter out (lower analytical value)
...
def get_agent_names_from_config(config: Dict[str, Any]) -> Dict[int, str]:
    ...
    This allows showing real entity names in actions.jsonl instead of aliases like "Agent_0"
...
def fetch_new_actions_from_db(
    ...
    """
    Fetch new action records from database with full context info
    
    Args:
        db_path: Database file path
        last_rowid: Max rowid from last read (using rowid instead of created_at due to format differences across platforms)
    ...
    try:
        ...
        # This avoids created_at format difference issues (Twitter uses integers, Reddit uses datetime strings)
        cursor.execute("""
...
def _enrich_action_context(
    ...
    """
    Enrich actions with context info (post content, usernames, etc.)
    ...
    try:
        ...
        # Repost: add original post content and author
            ...
            if new_post_id:
                # Reposted post's original_post_id points to original
                cursor.execute("""
        ...
        # Create comment: add info about the commented post
...
def _get_post_info(
    ...
    try:
        ...
        if row:
            ...
            # Prefer using name from agent_names
            author_name = ''
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    Supports dual LLM config for speeding up parallel simulations:
    ...
    If boost LLM is configured, parallel simulations can use different API providers for different platforms.
    ...
    # Select which LLM to use based on parameters and config
...
async def run_twitter_simulation(
    ...
    # Twitter uses standard LLM config
...
async def run_reddit_simulation(
    ...
    # Reddit uses boost LLM config (if available, falls back to standard)
...
async def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help='Max simulation rounds (optional, for truncating long simulations)'
    )
    parser.add_argument(
        '--no-wait',
        action='store_true',
        default=False,
        help='Close environment immediately after simulation, skip command waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at start of main, ensuring entire program can respond to exit signals
    ...
    # Initialize logging config (disable OASIS logs, clean old files)
    ...
    log_manager.info(f"  - Total simulation duration: {total_hours}h")
...
def setup_signal_handlers(loop=None):
    ...
    Persistent simulation: do not exit after simulation, wait for interview commands
    ...
    def signal_handler(signum, frame):
        ...
        if not _cleanup_done:
            _cleanup_done = True
            # Set event to notify asyncio loop to exit (giving loop a chance to clean up)
            if _shutdown_event:
                _shutdown_event.set()
        
        # Do not sys.exit() directly, let asyncio loop exit normally and clean up
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Clean up multiprocessing resource tracker (prevent exit warnings)
        try:
