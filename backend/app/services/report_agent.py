...
class ReportLogger:
    ...
    def log_section_content(
        ...
        """Record section content generation complete (records content only, does not mean entire section is complete)"""
        self.log(
...
[Panoramic Search - Get Full Picture View]
...
This is not LLM simulation - it calls the real interview interface to get simulation Agents' original responses.
...
[Use Cases]
- Need to understand event perspectives from different roles (How do students see it? What does media think? What does the official say?)
...
[Core Concept]
We have built a simulated world and injected specific "simulation requirements" as variables. The evolution results of the simulated world are predictions of what might happen in the future. What you are observing is not "experimental data", but a "preview of the future".

[Your Task]
Write a "Future Prediction Report" that answers:
1. Under the conditions we set, what happened in the future?
2. How did various Agents (groups) react and act?
3. What notable future trends and risks does this simulation reveal?
...
- Agent behavior in the simulated world is a prediction of future crowd behavior
- NOT an analysis of real-world current conditions
- NOT a generic public opinion overview

[Section Count Limits]
- Minimum 2 sections, maximum 5 sections
- No subsections needed, each section directly contains complete content
...
Please output a JSON-format report outline as follows:
{
    "title": "Report title",
...
3. What notable future trends does this simulation reveal?
...
DO focus on "what the future will look like" - simulation results ARE the predicted future
...
2. [Must quote Agents' original words and actions]
   - Agent statements and behavior are predictions of future crowd behavior
...
3. [Language consistency - quoted content must be translated to the report language]
   ...
   - Maintain original meaning during translation, ensure natural and smooth expression
   - This rule applies to both body text and quote blocks (> format)

4. [Faithfully present prediction results]
   - Report content must reflect simulation results representing the future from the simulated world
   - Do not add information that does not exist in the simulation
   - If information is insufficient in certain aspects, state so honestly
...
Weibo, as the first scene of public opinion, assumed the core function of initial information dissemination:
...
[Tool Usage Tips - Please mix different tools, don't use only one]
...
- panorama_search: Panoramic search, understand the full picture of events, timeline, and evolution
- quick_search: Quickly verify specific information points
- interview_agents: Interview simulation Agents, get first-person perspectives and authentic reactions from different roles

═══════════════════════════════════════════════════════════════
[Workflow]
═══════════════════════════════════════════════════════════════

Each reply you can only do one of the following two things (not both simultaneously):
...
4. [Quote Format - Must be standalone paragraphs]
   Quotes must be standalone paragraphs with a blank line before and after, cannot be mixed within paragraphs:
...
- If information is sufficient: output section content starting with "Final Answer:" (must quote the above text)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately output section content based on obtained information, starting with "Final Answer:".'
)

REACT_UNUSED_TOOLS_HINT = "\nTip: You haven't used these yet: {unused_list}, recommend trying different tools for multi-perspective information"
...
class ReportAgent:
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            if tool_name == "insight_forge":
                ...
                # Panoramic search - get full picture
                query = parameters.get("query", "")
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only tried when format 1 doesn't match, to avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        # Build user prompt - pass in up to 4000 chars from each completed section
        ...
        used_tools = set()  # Track names of already-called tools
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                if iteration < max_iterations - 1:
                    ...
                    continue
                # Last iteration also returns None, break loop to force wrap-up
                break
            ...
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    ...
                    messages.append({
                        "role": "user",
                        "content": (
                            "[Format Error] You included both tool calls and Final Answer in a single reply, which is not allowed.\n"
            ...
            if has_final_answer:
                # Insufficient tool calls, reject and require continuing tool calls
                if tool_calls_count < min_tool_calls:
                    messages.append({"role": "assistant", "content": response})
                    unused_tools = all_tools - used_tools
                    unused_hint = f"(These tools haven't been used yet, recommend trying them: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",
        ...
        # Check if LLM returns None during forced wrap-up
        if response is None:
    ...
    def generate_report(
        ...
        """
        Generate complete report (real-time output per section)
        
        Each section is saved to folder immediately upon completion, no need to wait for the entire report.
        File structure:
        reports/{report_id}/
...
class ReportManager:
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        Args:
            report_id: Report ID
            from_line: Starting line to read from (for incremental retrieval, 0 means from beginning)
    ...
    def save_section(
        ...
        Called immediately after each section is generated, implementing per-section output
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        1. Remove Markdown heading lines at content beginning that duplicate the section title
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate heading (same title appearing within consecutive 5 lines)
                is_duplicate = False
                ...
                if level == 1:
                    if title == outline.title:
                        ...
                        # Section title incorrectly uses #, fix to ##
