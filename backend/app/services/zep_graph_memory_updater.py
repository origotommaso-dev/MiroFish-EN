...
class AgentActivity:
    ...
    def to_episode_text(self) -> str:
        """
        Convert activity into a text description that can be sent to Zep
        ...
        # Return "agent_name: activity_description" format directly, without simulation prefix
    ...
    def _describe_create_comment(self) -> str:
        """Create comment - includes comment content and the post being commented on"""
        content = self.action_args.get("content", "")
        post_content = self.action_args.get("post_content", "")
        post_author = self.action_args.get("post_author_name", "")
    ...
    def _describe_mute(self) -> str:
        """Mute user - includes the muted user's name"""
        target_user_name = self.action_args.get("target_user_name", "")
...
class ZepGraphMemoryUpdater:
    ...
    Groups by platform; sends batch to Zep after accumulating BATCH_SIZE activities.
    ...
    # Send interval (seconds) to avoid requests too fast
    SEND_INTERVAL = 0.5
    ...
    def _worker_loop(self):
        """Background worker loop - batch send activities to Zep per platform"""
