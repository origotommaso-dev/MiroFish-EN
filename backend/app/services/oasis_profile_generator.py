...
Converts entities from Zep graph to Agent Profile format required by the OASIS simulation platform
...
class OasisProfileGenerator:
    ...
    Converts entities from Zep graph to Agent Profiles needed for OASIS simulation
    ...
    # Individual type entities (require specific persona generation)
    INDIVIDUAL_ENTITY_TYPES = [
        "student", "alumni", "professor", "person", "publicfigure", 
        "expert", "faculty", "official", "journalist", "activist"
    ]
    
    # Group/institutional type entities (require representative persona generation)
    GROUP_ENTITY_TYPES = [
    ...
    def generate_profile_from_entity(
        ...
        if use_llm:
            ...
            # Use rules for basic persona generation
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        ...
        Zep does not have a built-in hybrid search interface; edges and nodes must be searched separately and results merged.
        Uses parallel requests for simultaneous searching to improve efficiency.
    ...
    def _is_individual_entity(self, entity_type: str) -> bool:
        """Determine if this is an individual type entity"""
        return entity_type.lower() in self.INDIVIDUAL_ENTITY_TYPES
    
    def _is_group_entity(self, entity_type: str) -> bool:
        """Determine if this is a group/institutional type entity"""
    ...
    def _generate_profile_with_llm(
        ...
        """
        Generate very detailed persona using LLM
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if after the last quote there is no comma or closing bracket, string may be truncated
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are a social media user profile generation expert. Generate detailed, realistic personas for public opinion simulation, restoring existing real-world situations as much as possible. Must return valid JSON format, all string values must not contain unescaped newlines. Use Chinese."
        return base_prompt
...
2. persona: Detailed persona description (2000 characters of plain text), must include:
   ...
   - Stance and opinions (attitude toward topics, content that may provoke/move them)
   - Unique features (catchphrases, special experiences, personal hobbies)
   - Personal memory (important part of persona, introduce this individual's connection to events, and their existing actions and reactions in events)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        return f"""Generate a detailed social media account profile for an institutional/group entity, restoring existing real-world situations as much as possible.
...
2. persona: Detailed account profile description (2000 characters of plain text), must include:
   - Institution basic info (official name, nature, establishment background, main functions)
   ...
   - Institutional memory (important part of persona, introduce this institution's connection to events, and its existing actions and reactions in events)
...
- Institutional account communications must match its identity positioning"""
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for realtime writing (if provided, writes after each generation)
        ...
        def generate_single_profile(idx: int, entity: EntityNode) -> tuple:
            ...
            try:
                ...
                # Output generated persona to console and logs in realtime
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        """
        Save Twitter Profile in CSV format (per OASIS official requirements)
