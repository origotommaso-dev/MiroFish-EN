...
def create_simulation():
    ...
    Note: Parameters like max_rounds are intelligently generated by the LLM and do not need manual setting.
...
def prepare_simulation():
    ...
    Steps:
    1. Check if preparation has already been completed
    ...
    try:
        ...
        # This way the frontend can get the expected total Agent count immediately after calling prepare
        try:
            ...
            filtered_preview = reader.filter_defined_entities(
                ...
                enrich_with_edges=False  # Skip edge info for faster execution
            )
...
def get_prepare_status():
    ...
    try:
        ...
        if not task:
            # Task not found, but if simulation_id exists, check if preparation is complete
            if simulation_id:
                is_prepared, prepare_info = _check_simulation_prepared(simulation_id)
                if is_prepared:
                    return jsonify({
...
def get_simulation_history():
    ...
    try:
        ...
        # Enrich simulation data, only reading from Simulation files
        ...
        for sim in simulations:
            ...
            if run_state:
                ...
                # Use user-configured total_rounds; if unavailable, use recommended rounds
...
def get_simulation_profiles_realtime(simulation_id: str):
    ...
    try:
        ...
        # Check if generation is in progress (by checking state.json)
        is_generating = False
        total_expected = None
...
def get_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": f"Simulation config not found, please call /prepare endpoint first"
            }), 404
...
def start_simulation():
    ...
    About the force parameter:
        - When enabled, if simulation is running or completed, it will stop and clean run logs first
...
def get_run_status_detail(simulation_id: str):
    ...
    try:
        ...
        # recent_actions only shows the latest round's content from both platforms
...
def interview_agent():
    ...
    Note: This feature requires the simulation environment to be running (entered command-waiting mode after completing simulation loop).
    ...
    Request (JSON):
        {
            ...
            "interviews": [                    // Required, interview list
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your view on B?"  // If platform not specified, uses default
                }
            ],
...
def close_simulation_env():
    ...
    Sends a close environment command to the simulation for graceful exit from command-waiting mode.
